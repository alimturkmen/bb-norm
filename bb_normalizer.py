from entity import Biotope, SearchEntity, SynonymType, EntityType
import defs
...
class ExactMatch(object):
    ...
    def match_term(self, term:SearchEntity, weighted=False):
        ...
        if term.type == EntityType.microorganism : return {'type':'N', 'annotation':term.id, 'ref':'727'}
        elif term.type == EntityType.phenotype : return {'type':'O', 'annotation':term.id, 'ref':'002762'}
        elif weighted : return self.weighted_match_term(term)
        matched_id = '003381'
        ...
        for id in self.ontobiotope:
            ...
            if term.name.lower() == biotope.name : return {'type':'O', 'annotation':term.id, 'ref':id} # try lowercased-term
            if term.name[0:-1] == biotope.name : return {'type':'O', 'annotation':term.id, 'ref':id} # in case plural from of term
    ...
    def weighted_match_term(self, term:SearchEntity):
        ...
        for id in self.ontobiotope:
            ...
            for name in term.name_list:
                if len(name) > 0:
                    if (name in biotope.name_list or name[0].lower()+name[1:] in biotope.name_list or name[0:-1] in biotope.name_list):
    ...
    for i, prediction_list in enumerate(predictions):
        ...
        with open(dir+file_name, 'w') as file:
            for j, prediction in enumerate(prediction_list):
                ...
                ref = ' Referent:OBT:'+prediction['ref']+'\n' if prediction['type'] == 'O' else ' Referent:'+prediction['ref']+'\n'
                line = 'N' + str(j+1) + typ + annotation + ref
                file.write(line)


def create_eval_file_for_context(predictions:Dict[str, List[dict]], file_path: str):
    pass
    dir = defs.OUTPUT_PATH
